...
class Step:
    def __init__(self, name: str, action: Callable[[None], str]):
        self.name = name
        self.action = action

    def execute(self) -> bool:
        Utils.printInline(self.name)
        errorStr = self.action()
...
class Utils:
    ...
    def getPackagesLockJsonPath(projectPath: str) -> str:
        return Utils.getUnityPackagesPath(projectPath) + "/packages-lock.json"
    ...
    def getPackageFullName(packageVersion: PackageVersion) -> str:
        return Utils.PACKAGE_NAME + "@" + Utils.getPackageVersionStr(packageVersion)
    ...
    def getPackagePath(
        ...
        if inCache:
            return os.path.join(
                ...
                Utils.getPackageFullName(packageVersion),
            )
        else:
            return os.path.join(
                Utils.getUnityPackagesPath(projectPath), Utils.getPackageFullName(packageVersion)
            )
